...
async def test_skips_observers_for_missing_optional_clis() -> None:
    ...
    with (
        ...
        patch("ductor_bot.orchestrator.observers.OpencodeCacheObserver") as opencode_cls,
    ...
    assert manager.opencode_cache_obs is None
...
async def test_starts_observers_for_installed_optional_clis() -> None:
    ...
    opencode_observer = MagicMock()
    opencode_observer.start = AsyncMock()
    ...
    with (
        ...
        patch(
            "ductor_bot.orchestrator.observers.OpencodeCacheObserver",
            return_value=opencode_observer,
        ),
        ...
        await manager.init_model_caches(
            installed_providers=frozenset(
                {"claude", "codex", "gemini", "antigravity", "grok", "opencode"}
            ),
            on_gemini_refresh=MagicMock(),
            on_antigravity_refresh=MagicMock(),
            on_grok_refresh=MagicMock(),
    ...
    opencode_observer.start.assert_awaited_once()
    ...
    assert manager.opencode_cache_obs is opencode_observer
