...
def _status_effort_suffix(orch: Orchestrator, model_name: str) -> str:
...
async def _build_status(orch: Orchestrator, key: SessionKey) -> str:
    ...
    if session:
        ...
        session_block = (
            ...
            f"{_model_line(session.model)}{_status_effort_suffix(orch, session.model)}"
        )
    else:
        session_block = (
            f"{t('status.no_session')}\n"
            f"{_model_line(runtime_model)}{_status_effort_suffix(orch, runtime_model)}"
        )
