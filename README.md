...
- **Rename, new folder, delete, move and copy** — behind a `🛠 Manage` button, so nothing that changes files sits on the row you tap to browse. Renaming and creating ask for the name as a message and show it back for approval before writing. Deleting takes two confirmations and states what will go — file count and real size, not a rounded one. Two cases are refused outright rather than confirmed: a configured project root, and any directory containing `.git`, whose untracked files exist nowhere else. Move and copy mark a source and paste it into whichever folder you open next, and refuse outright rather than confirm in four cases: an existing name at the destination (nothing is ever overwritten), a folder pasted inside its own subtree, a configured root being moved, and a destination outside the browsable folders
- **Copy path** (`📋 Path`) — the absolute path of any file or folder, in a tap-to-copy code span, so a location can be pasted into a prompt without sending the file anywhere. Each path gets its own block, because a tap copies the whole block and a folder's worth of paths at once is not something anyone pastes. The path is the real one on disk, not the browser's display label, which would send an agent to a directory that does not exist
...
| `/files` | Browse `~/.ductor/` and your configured `project_roots`; download, upload, rename, create, delete, move, copy, show absolute paths, and pull/push git. `/showfiles` still works as an alias |
