...
- **The bot's own files stay findable** — a conversation running in a project directory is still told where ductor's workspace is, by absolute path. Prompts that name `tools/`, `memory_system/`, `user_tools/` or `cron_tasks/` are anchored to `~/.ductor/workspace`, so an agent working inside your repo does not go looking for them under it and conclude it is in the wrong place
